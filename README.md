...
현재는 다른 도메인 없이 User 회원가입에 대해서만 쿼리스트링이 들어오기 때문에 많은 클래스 (Controller 등등)로 세분화하지 않았다. 이에 대해 고민을 하였으나 현재 요구사항에서는 더 이상 구현을 추가하지 않고 추후 웹서버의 다양한 요구조건이 생기면 처리할 예정이다.(물론 그룹 피드백을 거치며 추가하기로 바꿀 수도 있다..)
1번과 2번의 과정은 String 클래스에서 제고아는 split
